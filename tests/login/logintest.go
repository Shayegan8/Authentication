...
func main() {
	...
	bucket1, _ := http.Get("http://127.0.0.1:1234/login")

	// GET
	bucket1Cookie := bucket1.Cookies()[0]
	vals := strings.Split(bucket1Cookie.Value, ",")
	csrf_token := vals[1]
	l("/login GET status", bucket1.Status)
	bucket1.Body.Close()

	// POST
	toRegister, _ := http.NewRequest("POST", "http://127.0.0.1:1234/login", nil)
	...
	// GET
	bucket2, _ := http.Get("http://127.0.0.1:1234/login/validation")
	bucket2Cookie := bucket2.Cookies()[0]
	vals = strings.Split(bucket2Cookie.Value, ",")
	csrf_token = vals[1]
	l("/login/validation GET status", bucket2.Status)
	...
	toRegisterValidation, _ := http.NewRequest("POST", "http://127.0.0.1:1234/login/validation", nil)
	println("hereee")
	...
	// GET
	bucket3, _ := http.Get("http://127.0.0.1:1234/login/validation/jwt")
	bucket3Cookie := bucket3.Cookies()[0]
	vals = strings.Split(bucket3Cookie.Value, ",")
	csrf_token = vals[1]
	l("/login/validation/jwt GET status", bucket3.Status)

	bucket3.Body.Close()

	// POST
	toRegisterValidationJWT, _ := http.NewRequest("POST", "http://127.0.0.1:1234/login/validation/jwt", nil)
	...
	// GET
	bucket4, _ := http.Get("http://127.0.0.1:1234/login/validation/submit")
	bucket4Cookie := bucket4.Cookies()[0]
	vals = strings.Split(bucket4Cookie.Value, ",")
	csrf_token = vals[1]
	l("/login/validation/submit GET status", bucket4.Status)

	bucket4.Body.Close()

	// POST
	toRegisterValidationSubmit, _ := http.NewRequest("POST", "http://127.0.0.1:1234/login/validation/submit", nil)
